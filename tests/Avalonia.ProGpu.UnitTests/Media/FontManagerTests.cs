...
namespace Avalonia.ProGpu.UnitTests.Media
{
    ...
    {
        private static string s_fontUri = "resm:Avalonia.ProGpu.UnitTests.Assets?assembly=Avalonia.ProGpu.UnitTests#Noto Mono";
        ...
        public void Should_Throw_For_Invalid_Custom_Font()
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                Assert.Throws<InvalidOperationException>(() => new Typeface("resm:Avalonia.ProGpu.UnitTests.Assets?assembly=Avalonia.ProGpu.UnitTests#Unknown").GlyphTypeface);
            }
        }
        ...
        public void Should_Return_False_For_Invalid_DefaultFontFamily()
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                using (AvaloniaLocator.EnterScope())
                {
                    AvaloniaLocator.CurrentMutable.BindToSelf(new FontManagerOptions { DefaultFamilyName = "avares://resm:Avalonia.ProGpu.UnitTests.Assets?assembly=Avalonia.ProGpu.UnitTests#Unknown" });
        ...
        public void Should_Create_Synthetic_Typeface()
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                using (AvaloniaLocator.EnterScope())
                {
                    ...
                    Assert.NotEqual(((ProGpuTypeface)regularTypeface.PlatformTypeface).Font, ((ProGpuTypeface)italicBoldTypeface.PlatformTypeface).Font);
                }
            }
        }
        ...
        public void Should_Get_GlyphTypeface_By_Localized_FamilyName()
        {
            using (UnitTestApplication.Start(
                       TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                using (AvaloniaLocator.EnterScope())
                {
                ...
                using (AvaloniaLocator.EnterScope())
                {
                    FontManager.Current.AddFontCollection(
                        new EmbeddedFontCollection(
                            ...
                            new Uri("resm:Avalonia.ProGpu.UnitTests.Assets?assembly=Avalonia.ProGpu.UnitTests"))); //source
        ...
        public void Should_Get_SystemFont_With_BaseUri(string name)
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                using (AvaloniaLocator.EnterScope())
                {
                    var fontFamily = new FontFamily(new Uri("avares://Avalonia.ProGpu.UnitTests/NotFound"), name);
        ...
        public void Should_Get_Regular_Font_After_Matching_Italic_Font()
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                using (AvaloniaLocator.EnterScope())
                {
                    ...
                    Assert.NotEqual(((ProGpuTypeface)italicTypeface.GlyphTypeface.PlatformTypeface).Font, ((ProGpuTypeface)regularTypeface.GlyphTypeface.PlatformTypeface).Font);
                }
            }
        }
        ...
        {
            const string fontUri = "resm:Avalonia.ProGpu.UnitTests.Fonts.TestFontNoCmap412.ttf?assembly=Avalonia.ProGpu.UnitTests#TestFontNoCmap412";
