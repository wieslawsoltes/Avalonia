...
namespace Avalonia.ProGpu.UnitTests.Media
{
    ...
    {
        private const string AssetsNamespace = "Avalonia.ProGpu.UnitTests.Assets";
        private const string AssetFonts = $"resm:{AssetsNamespace}?assembly=Avalonia.ProGpu.UnitTests";
        ...
        public void Should_AddGlyphTypeface_By_Stream()
        {
            using (UnitTestApplication.Start(TestServices.MockPlatformRenderInterface.With(fontManagerImpl: new CustomFontManagerImpl())))
            {
                ...
                for (var i = 0; i < infos.Length; ++i)
                {
                    ...
                    if (fontCollection.TryAddGlyphTypeface(fontStream, out var glyphTypeface))
                    {
                ...
                for (var i = 0; i < infos.Length; ++i)
                {
                    ...
                    if (glyphTypeface == null) continue;
