...
| --- | --- | --- |
| `ProGPU.Avalonia.Rendering` `12.0.5-preview.19` | `Avalonia.ProGpu` | ProGPU/WebGPU rendering backend |
| `ProGPU.Avalonia.SilkNet` `12.0.5-preview.19` | `Avalonia.SilkNet` | Cross-platform Silk.NET windowing backend |

Both packages are built against exactly Avalonia `12.0.5` and ProGPU `0.1.0-preview.21`. They intentionally use `ProGPU.*` package IDs; no `Avalonia.*` package ID is produced by this release lane.
...
Publish ProGPU `0.1.0-preview.21` first, then pack the integrations:
...
1. Tag and publish ProGPU `0.1.0-preview.21`.
...
4. Publish `ProGPU.Avalonia.Rendering` and `ProGPU.Avalonia.SilkNet` `12.0.5-preview.19`.
